...
in scripts/validate_ml5_results.py instead.
...
from run_ml5_stage_b import (BASELINES, CFG5, DRAFT_SIZE,  # noqa: E402
                             SELECTED_MODEL, SELECTED_TARGET,
                             b5a_global_mean_pick, b5b_position_mean_pick,
                             feature_set, fit_predict, load_stage_b,
                             make_estimator, make_pipeline, stage_b_metrics,
                             strength, tier_metrics, tier_of, to_pick,
                             to_target)
...
class TestStageBPopulation(unittest.TestCase):
    ...
    def test_undrafted_prospects_are_removed_not_relabelled(self):
        ...
        from ml3_common import load_development
...
class TestFeatureAndTargetSeparation(unittest.TestCase):
    ...
    def test_stage_a_representation_is_inherited_unchanged(self):
        fa = CFG5["frozen_stage_a_representation"]
        self.assertEqual(fa["feature_set"], "SET_2_BOX_SHOT_PROFILE")
        self.assertEqual(fa["normalization"], "SEASON_RELATIVE")
...
class TestHoldoutFirewall(unittest.TestCase):
    def test_run_script_never_loads_holdout_targets(self):
        """Only the run script is scanned: validate_ml5_results.py contains
    ...
    def test_validator_enforces_the_holdout_ban(self):
        src = (ROOT / "scripts" / "validate_ml5_results.py").read_text()
