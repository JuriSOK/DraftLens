...
from draftlens.comparables.explanations import explain_pair
from draftlens.comparables.nba_features import PAIRS, TOTALS, build_metrics, parse_pair
...
CONFIG = json.loads(
    (ROOT / "config" / "features" / "nba_comparables.json").read_text())
...
class TestNbaSourceParsing(unittest.TestCase):
    ...
    def test_metrics_use_the_shared_formulas(self):
        """efg/ts must come from draftlens.features, not a second copy."""
        src = (ROOT / "src" / "draftlens" / "comparables"
               / "nba_features.py").read_text()
        self.assertIn("from draftlens.features.shooting import", src)
        self.assertIn("from draftlens.features.rates import", src)
...
class TestSimilarityScore(unittest.TestCase):
    ...
    def test_score_is_not_a_linear_rescale_of_distance(self):
        ...
        import ast
        import inspect
        from draftlens.comparables import similarity as sim
