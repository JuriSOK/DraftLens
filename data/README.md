...
Source data is **acquired locally, never committed**. Reproducibility comes from the acquisition scripts plus [`source_manifest.csv`](source_manifest.csv), not from redistributing source files.
...
./.venv/bin/python scripts/acquire.py all --years 2011-2026

# Wikipedia prospect population + draft targets, and 2026 Wikidata DOB
./.venv/bin/python scripts/acquire.py population --years 2011-2026 --wikidata
...
Files here are **never modified in place**. If a source file is wrong or needs cleaning, the fix belongs in a transformation script writing to `interim/` — not in an edit to the raw file. `scripts/validate.py --stage raw` compares every file's SHA-256 against the manifest and fails if a raw file changed after acquisition.
...
`draft_population/` and `draft_targets/` are **deliberately separate directories** and must never be merged into one convenient file before the feature-generation boundary.
...
`scripts/validate.py --stage raw` asserts that no outcome column ever appears in a population file.

**Two population fields are prohibited as model features**: `early_entrant` and `population_source`. Across 2011–2026, 212 of 212 non-early-entrants were drafted, so `early_entrant = False` predicts the target with certainty. They are provenance metadata, not inputs.

**Date of birth is display-only**. It lives in `wikidata/`, never in a feature file, and must not enter General Draft Board historical features — including as a missingness indicator.
...
- **Do not commit raw data.** `.gitignore` excludes `data/**` except this README, the manifest. Verify with `git check-ignore` before committing.
...
- **Do not fabricate missing data.** Missing values may only be filled by an explicitly documented and approved method.
...
2011–2026 acquired and validated: 168,461 NCAA player-season rows, 2,857,536 player-game rows, 12,724,485 shots, 8,342 NBA player-seasons, and 1,186 reconstructed draft prospects. See [DATA.md](../docs/DATA.md) for the full source profile.
