"""Data acquisition: HTTP access, the source manifest, and raw-corpus
validation.

Every DraftLens source is public and unauthenticated. This module never sends
credentials, never bypasses rate limits, and identifies itself honestly. It
also owns the source manifest — provenance (URL, timestamp, size, SHA-256) for
every acquired raw file, so a reviewer can confirm the raw corpus was not
silently edited. Raw data is immutable; the manifest is how that is checked.

Source-specific parsing lives in sibling modules (`hoopr`, `wikipedia`,
`wikidata`) because each is substantial enough to be independently testable;
...
from paths import MANIFEST, MBB, RAW, ROOT, rel
...
# Checks structure, coverage, parseability, manifest integrity (files
# unmodified since acquisition), the feature/target firewall, and cross-season
# athlete_id stability. Computes no model features. This is the ONE opt-in
# validation stage that needs the full raw corpus on disk (~200 MB), so it is
# not part of the default `scripts/validate.py` run.
FAIL, WARN = [], []
...
def validate_manifest(skip_checksums):
    _rule("MANIFEST INTEGRITY")
    man = load_manifest()
    _check(len(man) > 0, "source_manifest.csv is empty")
...
def validate_mbb():
    ...
    for y in YEARS:
        ...
        _check(len(core) > 0, f"MBB {y}: player_core empty")
        _check(len(box) > 0, f"MBB {y}: player_box empty")
        _check(len(shots) > 0, f"MBB {y}: shots empty", hard=False)
        ...
        _check(need_core <= set(core.columns),
              f"MBB {y}: player_core missing {sorted(need_core - set(core.columns))}")
        _check(need_box <= set(box.columns),
              f"MBB {y}: player_box missing {sorted(need_box - set(box.columns))}")
        _check(need_shots <= set(shots.columns),
              f"MBB {y}: shots missing {sorted(need_shots - set(shots.columns))}")

        cid, bid = _ids(core.athlete_id), _ids(box.athlete_id)
        _check(len(cid) == len(core), f"MBB {y}: player_core athlete_id not numeric-safe")
        cover = len(set(bid) & set(cid)) / max(1, len(set(bid)))
        _check(cover > 0.95, f"MBB {y}: box->core id coverage only {100*cover:.1f}%",
              hard=False)
...
def validate_population():
    _rule("DRAFT POPULATION & TARGETS — coverage, firewall, class balance")
    rows = []
    for y in YEARS:
        pp, tp = POP / f"draft_population_{y}.csv", TGT / f"draft_targets_{y}.csv"
        if not _check(pp.exists() and tp.exists(), f"draft {y}: population/target missing"):
            continue
        pop = pd.read_csv(pp)
        tgt = pd.read_csv(tp)
        ...
        _check(not leaked, f"FIREWALL BREACH {y}: outcome columns in population: {leaked}")
        _check("pick" in tgt.columns, f"draft {y}: targets missing pick column")

        _check(pop.normalized_name.duplicated().sum() == 0,
              f"draft {y}: duplicate normalized_name in population")
        _check(set(pop.normalized_name) == set(tgt.normalized_name),
              f"draft {y}: population/target key mismatch")
        _check(len(pop) > 0 and (pop.early_entrant.astype(str).isin(
        ...
        _check(picks.notna().all(), f"draft {y}: drafted rows with null pick")
        _check(picks.between(1, 60).all(), f"draft {y}: pick outside 1-60", hard=False)
...
def id_stability():
    _rule("CROSS-SEASON athlete_id STABILITY (hoopR MBB player_core)")
    ...
    _check(100 * stable / max(1, len(multi)) > 95,
...
def validate_2026(mbb_df):
    ...
    _check(len(wd) == len(pop), "2026: Wikidata rows != population rows")
    _check("pick" not in pop.columns and "drafted" not in pop.columns,
          "2026: firewall breach in population file")
    _check("date_of_birth" not in pop.columns, "2026: DOB leaked into population file")
    ...
    print(f"  Wikidata DOB (display-only)   : {dob}/{len(wd)} "
          f"({100*dob/len(wd):.1f}%)")
    _check(hit / len(pop) > 0.9, f"2026 hoopR match rate {100*hit/len(pop):.1f}% < 90%")
    return dict(population=len(pop), matched=hit, dob=dob)


def validate_raw(skip_checksums=False):
    """The opt-in raw-corpus validator: `scripts/validate.py --stage raw`."""
    global FAIL, WARN
    FAIL, WARN = [], []
    validate_manifest(skip_checksums)
    mbb = validate_mbb()
    validate_nba()
    validate_population()
    id_stability()
