...
from draftlens.comparables.nba_features import load_player_seasons
from draftlens.paths import CONFIG_FEATURES, interim

CONFIG_PATH = CONFIG_FEATURES / "nba_comparables.json"
...
def build_ncaa_reference(years, metrics=None, min_minutes=NCAA_MIN_MINUTES,
    ...
    """
    from draftlens.comparables.space import COMMON_METRICS
    from draftlens.team_need.reference import _season_frame
    metrics = metrics if metrics is not None else COMMON_METRICS
...
def load_ncaa_reference(path=NCAA_REFERENCE_FILE):
    if not path.exists():
        raise FileNotFoundError(
            f"{path} missing — run scripts/build_comparable_references.py")
    return pd.read_parquet(path)
