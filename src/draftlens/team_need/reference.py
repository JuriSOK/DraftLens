...
from draftlens.features.boxscore import aggregate_box_frame
from draftlens.features.engineering import build_features, team_context
from draftlens.features.positions import to_position_3
from draftlens.paths import MBB, interim
...
def _season_frame(year):
    ...
    from draftlens.features.shot_profile import aggregate_shots_frame
    sh = pd.read_parquet(
...
def load_reference(path=REFERENCE_FILE):
    if not path.exists():
        raise FileNotFoundError(
            f"{path} missing — run scripts/build_team_need_reference.py")
