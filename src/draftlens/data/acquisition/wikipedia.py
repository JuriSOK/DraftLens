...
`draftlens.data.population.load_population` filters to `early_entrant == True`.
The earlier union rule (DEC-038) was superseded in ML-0.1 because population
...
def acquire(years, wikidata=False):
    ...
    """
    from draftlens.data.acquisition.manifest import (load_manifest,
                                                     manifest_record,
                                                     sha256_file, utcnow,
                                                     write_manifest)
    from draftlens.data.acquisition.wikidata import WD_DIR, enrich_wikidata
    from draftlens.paths import rel
