...
validation year's distribution backwards into the training years (ML_SPEC 9.2).

Missing-data policy (DEC-069, DEC-073): train-fold median imputation, no
missingness indicator columns, and no complete-case deletion. A missingness
indicator is itself a feature and would carry the outcome — DOB demonstrated
that it would be the most target-predictive column in the dataset while holding
no basketball information.
"""
...
REFERENCE = INTERIM / "ml2" / "ncaa_reference_distributions.parquet"
...
def make_pipeline(feats, strategy, position, scaling, class_weight, C, seed):
    """ML-3 logistic pipeline. Refitted inside every fold, never reused."""
    from sklearn.linear_model import LogisticRegression
    pre = build_preprocessor(feats, position, scaling)
...
def season_relative(df, feats, covered=None):
    ...
    the SAME season and coarse position (DEC-081).

    Leakage-safe on three counts, each verified in ML4_REPORT 12:
