...
finding worth reading; a FAIL means the phase result must not be trusted.
  ...
  ./.venv/bin/python scripts/validate_ml4_results.py
"""
...
def main():
    ...
    if missing:
        print(f"  FAIL  ML-4 outputs missing {missing} — run run_ml4_stage_a.py")
        return 1
    ...
    rerun = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "run_ml4_stage_a.py")],
