...
result must not be trusted.
  ...
  ./.venv/bin/python scripts/experiments/validate_ml5_stage_b.py
"""
...
from ml5_stage_b_selection import (CFG5, DRAFT_SIZE, OUT, SELECTED_MODEL,
...
def main():
    ...
    if missing:
        print(f"  FAIL  ML-5 outputs missing {missing} — run ml5_stage_b_selection.py")
        return 1
    ...
    for banned in ("targets_2026", "features_2026", "predictions_2026"):
        check(banned not in src, f"ml5_stage_b_selection.py references {banned}")
    ...
    rerun = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "experiments" / "ml5_stage_b_selection.py")],
