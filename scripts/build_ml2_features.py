"""ML-2 — derive the transparent basketball feature layer.

Consumes ML-0 outputs plus raw player_box (for team/opponent context) and writes
a new, separate ML-2 feature layer. ML-0 files are never mutated.

Respects the ML-1 constraints:
...
HOLDOUT GUARD: this script never opens a targets_*.parquet file. Feature values
for 2026 are produced by exactly the same formulas as development.

  ./.venv/bin/python scripts/build_ml2_features.py
  ./.venv/bin/python scripts/build_ml2_features.py --reference   (also build
      NCAA season x position reference distributions; slower)

Outputs (git-ignored) -> data/interim/ml2/
"""
...
from build_model_dataset import MBB, OUT as ML0, to_int_id  # noqa: E402
from ml2_formulas import (FT_POSSESSION_COEF, ast_pct, blk_pct,  # noqa: E402
                          ...
                          ts_pct, usage_pct)
from positions import UNKNOWN, to_position_3  # noqa: E402

OUT = Path(__file__).resolve().parents[1] / "data" / "interim" / "ml2"
DICT_PATH = (Path(__file__).resolve().parents[1] / "config"
             / "ml2_feature_dictionary.csv")
...
def main():
    ...
    ap.add_argument("--reference", action="store_true",
                    help="also build NCAA season x position reference "
                         "distributions from the full player population")
    a = ap.parse_args()
    OUT.mkdir(parents=True, exist_ok=True)
    rep, built = {}, {}

    for label, years in PARTITIONS.items():
        rule(f"PARTITION {label}")
        df = assemble(label, years)
        exp = EXPECTED_ROWS[label]
        ok = len(df) == exp
        log(f"  rows={len(df)} (expected {exp}) -> {'OK' if ok else 'MISMATCH'}")
        if not ok:
            raise SystemExit(f"population gate failed for {label}")
        feats = feature_columns(df)
        log(f"  engineered features: {len(feats)}")
        df.to_parquet(OUT / f"features_{label}.parquet", index=False)
        built[label] = df
        rep[label] = dict(rows=len(df), engineered_features=len(feats),
                          columns=len(df.columns))

    dev = built["2014_2025"]
    feats = feature_columns(dev)

    rule("COVERAGE OF ENGINEERED FEATURES (development)")
    cov = pd.DataFrame({"feature": feats,
                        "coverage_pct": [round(100 * dev[c].notna().mean(), 2)
                                         for c in feats]})
    log(cov.sort_values("coverage_pct").head(14).to_string(index=False))
    cov.to_csv(OUT / "feature_coverage.csv", index=False)
    rep["lowest_coverage"] = cov.sort_values("coverage_pct").head(8) \
        .to_dict("records")

    rule("RANGE VALIDITY")
    pcts = [c for c in feats if c.endswith("_pct") or c.endswith("_share")
            or c.endswith("_rate")]
    bad = {}
    for c in pcts:
        v = dev[c].dropna()
        if c.endswith(("_pct",)) and c not in ("ast_pct", "tov_pct", "orb_pct",
                                               "drb_pct", "trb_pct", "stl_pct",
                                               "blk_pct", "usage_pct"):
            n = int(((v < 0) | (v > 1)).sum())
        else:
            n = int((v < 0).sum())
        if n:
            bad[c] = n
    log(f"  out-of-range values: {bad if bad else 'none'}")
    inf = {c: int(np.isinf(dev[c].dropna()).sum()) for c in feats
           if np.isinf(dev[c].dropna()).any()}
    log(f"  infinities: {inf if inf else 'none'}")
    rep["out_of_range"] = bad
    rep["infinities"] = inf

    if a.reference:
        rule("NCAA REFERENCE DISTRIBUTIONS")
        ref = build_reference(sorted({y for ys in PARTITIONS.values()
                                      for y in ys}))
        ref.to_parquet(OUT / "ncaa_reference_distributions.parquet",
                       index=False)
        log(f"  rows={len(ref)}  "
            f"seasons={ref.season.nunique()}  metrics={ref.metric.nunique()}")
        rep["reference_rows"] = len(ref)

    (OUT / "quality_report.json").write_text(json.dumps(rep, indent=2,
                                                        default=str))
    rule("ML-2 RESULT")
    for k, v in rep.items():
        if isinstance(v, dict) and "rows" in v:
            log(f"  {k:<10} rows={v['rows']:<5} engineered={v['engineered_features']}")
    log(f"\n  outputs -> {OUT}")
    return 0
...
def build_reference(years):
    ...
    from build_model_dataset import aggregate_box_frame
    rows = []
